...
pub fn TryBase(T: type) type {
    return if (isBase(T)) T else @compileError(std.fmt.comptimePrint("Expected .int, .float, .bool, .@\"enum\", .@\"struct\", .vector (exclude Pointers) or []cosnt u8 type, found '{s}'", .{@typeName(T)}));
}
